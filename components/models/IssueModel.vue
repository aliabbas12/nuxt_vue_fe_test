...
const isOpenTranslationWithIssueModel = computed({
  get: () => modelStore.getIssueModelState,
  set: (value) => {
    modelStore.setIssueModelState(value);
  },
});
...
function proceedWithTranslation() {
  if (issueType.value === TranslationIssues.THREE_OR_MORE) {
    modelStore.setMoreThenTwoIssuesModelState(true);
  } else {
    modelStore.setSelectLanguageModelValue(true);
  }
  isOpenTranslationWithIssueModel.value = false;
}
const issueMessage = computed(() => {
  if (issueType.value === TranslationIssues.THREE_OR_MORE) {
    return "Oops! We found three or more issues we should fix before translating.";
  } else if (issueType.value === TranslationIssues.TWO) {
    return "Oops! We found two issues we should fix before translating.";
  } else if (issueType.value === TranslationIssues.ONE) {
    return "Oops! We found an issues we should fix.";
  } else {
    return "";
  }
});
      ...
      >
        ...
        <div class="w-full flex justify-center items-center flex-col py-3">
          ...
          <div class="my-4 w-full text-center flex justify-center items-center">
            <NextButton
              class="w-2/6"
              @call-event="isOpenTranslationWithIssueModel = false"
            />
          </div>
          <span
            class="text-black text-[14px] my-3 underline cursor-pointer"
